...
![Drifloon hex sticker](reference/figures/drifloon_hex.png)

Drifloon helps you discover and download historical hourly weather data
from Environment and Climate Change Canada (ECCC), then optionally build
a local SQLite database for analysis.
...
Before the downloads it is smart to look at the list of stations, as
each download requires either station name, station_id, or province.
...
By default, sync applies no year floor or year cap. Set min_year and/or
max_year only when you want explicit year filtering.

## Downloads

By default, downloads are written to a drifloon_output folder in your
working directory.
...
The function prompts for confirmation by default because this can be
large.
...
- Disk-space checks are warning-only; downloads continue even when free
  space appears low.

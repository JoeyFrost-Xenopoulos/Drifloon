...
test_that("download_station_by_name matches station_name case-insensitively", {
  captured_id <- NULL
  ...
  out_dir <- tempfile("drifloon-byname-case-")
  dir.create(out_dir)
...
test_that("download_station_by_name supports close fuzzy station_name matches", {
  captured_id <- NULL
  ...
  station_data <- data.frame(
    ...
    HLY.Last.Year = c(2005, 2005),
    stringsAsFactors = FALSE
  )

  out_dir <- tempfile("drifloon-byname-fuzzy-")
  dir.create(out_dir)
...
test_that("download_station_by_name uses station_id to disambiguate duplicate station names", {
  captured_id <- NULL
  captured_folder <- NULL

  fake_download_station <- function(station, ...) {
    captured_id <<- station$Station.ID
    captured_folder <<- station$Folder.Name
    invisible(NULL)
  }
  ...
  out_dir <- tempfile("drifloon-ambig-id-")
  dir.create(out_dir)
  ...
  expect_equal(captured_id, 11)
  expect_equal(captured_folder, "A_Station-11")
})
